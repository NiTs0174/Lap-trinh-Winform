...
namespace Lab02_02
{
    public partial class frmTimKiem : Form
    {
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            if (txtFullName.Text != null || txtStudentID != null)
            {
                try
                {
                    ketnoi.Open();
                    sql = "SELECT a.StudentID, FullName, FacultyName, AverageScore FROM Student a, Faculty b WHERE a.FacultyID = b.FacultyID AND (StudentID IS NULL OR StudentID like '" + txtStudentID.Text + "') AND (FullName IS NULL OR FullName like '%" + txtFullName.Text + "%') AND (FacultyName IS NULL OR FacultyName like N'" + cmbFaculty.Text + "')";
        ...
        {
            int Count = 0;
            for (int i = 1; i < dgvTimKiem.Rows.Count; i++)
            {
                Count++;
            }
